...
import renderSelectEditInputCell from "./Mark/SelectEditMarks.comp";
import { AppDispatch, StoreState } from "../../../types";

// TODO: make Sunday like free day
function generateDaysRange(lessonDate: Date) {
  let arr = [];
...
function LessonForm() {
  ...
  const { isLoading, activeLesson } = useSelector(
    (state: StoreState) => state.lesson
  );
  ...
  useEffect(() => {
    if (user.id) {
      dispatch(fetchLesson(Number(lessonId), user.id));
    }
  }, [user.id, dispatch]);
  ...
  function marksColor(params: { field: string; value?: number | null }) {
    if (
      params.field === "name" ||
      params.field === "id" ||
      params.value == null
